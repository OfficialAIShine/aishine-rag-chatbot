...
/**
 * Strip HTML tags and entities from formatted AI responses.
 * Used for sidebar conversation previews — the last_message field
 * contains formatted HTML from formatMessage(), not plain text.
 */
          ...
          {items.map((conv) => (
            <motion.div
              ...
              className={`group relative mx-2 rounded-lg cursor-pointer transition-all ${
                conv.id === currentConversationId ? activeItemStyle : itemStyle
              }`}
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className={`flex items-center justify-between p-3 border-b ${borderStyle}`}>
              <h2 className={`text-base font-semibold ${headingStyle}`}>Chats</h2>
              <button
                onClick={onClose}
            ...
            {/* Footer */}
            <div className={`p-3 border-t text-center ${borderStyle}`}>
