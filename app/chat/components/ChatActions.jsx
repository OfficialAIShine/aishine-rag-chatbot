...
export default function ChatActions({
  ...
  const divider = (
    <div className={`w-px h-4 mx-0.5 ${bedtimeMode ? 'bg-[#b8bdc4]' : 'bg-gray-300 dark:bg-gray-600'}`} />
  );
  ...
  return (
    ...
    >
      {/* Instagram-style heart on double click */}
      <AnimatePresence>
        {showHeart && (
          <motion.div
      ...
      <motion.button
        ...
        className={`p-1.5 rounded-md transition-all cursor-pointer ${buttonBase} ${
          feedback[messageIndex] === 'up'
            ? bedtimeMode ? 'bg-green-200 text-green-700' : 'bg-green-100 dark:bg-green-900/30 text-green-600'
            : iconColor
        }`}
        title="Helpful"
      >
      ...
      <motion.button
        ...
        className={`p-1.5 rounded-md transition-all cursor-pointer ${buttonBase} ${
          feedback[messageIndex] === 'down'
            ? bedtimeMode ? 'bg-red-200 text-red-700' : 'bg-red-100 dark:bg-red-900/30 text-red-600'
            : iconColor
        }`}
        title="Not helpful"
      >
      ...
      <motion.button
        ...
        className={`p-1.5 rounded-md transition-all cursor-pointer ${buttonBase} ${
          copied === messageIndex
            ? bedtimeMode ? 'text-green-700' : 'text-green-600'
            : iconColor
        }`}
        title="Copy"
      >
        {copied === messageIndex ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
      </motion.button>

      {/* Volume — hidden in compact (widget) mode */}
      {!isCompact && (
        <motion.button
          whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
          ...
          className={`p-1.5 rounded-md transition-all cursor-pointer ${buttonBase} ${
            speaking
              ? bedtimeMode ? 'text-blue-700' : 'text-blue-600'
              : iconColor
          }`}
          title="Read aloud"
        >
      ...
      {/* Share — hidden in compact (widget) mode */}
      {!isCompact && (
        <motion.button
          whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
      ...
      <motion.button
        ...
        className="flex items-center gap-1 p-1.5 rounded-md transition-all duration-300 cursor-pointer border border-purple-600 dark:border-purple-400 bg-purple-600 dark:bg-purple-400 text-white dark:text-gray-900 hover:bg-transparent hover:text-purple-600 dark:hover:text-purple-400"
        title="Tell me more"
      >
