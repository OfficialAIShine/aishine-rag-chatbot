...
/**
 * Strip HTML tags for plain-text sidebar previews.
 */
...
export default function ChatWidget({
  ...
  fontSize = 'medium',
  bubbleColor,
  // FIX: background is now received and applied to the messages area.
  // Previously this prop was never passed from page.js, and BACKGROUND_MAP
  // was never imported — so the background setting had zero effect in widget mode.
  background = 'moon',
}) {
  ...
  // FIX: Card no longer uses bg-white/95 — that was 95% opaque white covering
  // the background gradient entirely. Now uses bg-white/60 (60% white) so the
  // background gradient applied to the messages area is visible.
  const cardStyle = bedtimeMode
    ? 'bg-[#e0e5ec] shadow-[4px_4px_8px_#b8bdc4,-4px_-4px_8px_#ffffff]'
    : 'bg-white/60 dark:bg-gray-900/80 backdrop-blur-xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50';
  ...
  const inputStyle = bedtimeMode
    ...
    : 'bg-white/80 dark:bg-gray-800 text-gray-900 dark:text-white placeholder:text-gray-500';

  const buttonHoverStyle = bedtimeMode
    ? 'hover:bg-[#d1d6dc] text-[#1a0f08]'
    : 'hover:bg-black/8 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300';
  ...
  // FIX: Messages area background now uses BACKGROUND_MAP[background] via inline
  // style so the selected theme gradient is visible. Previously hardcoded to
  // bg-gray-50 class which ignored the background setting entirely.
  const messagesAreaStyle = bedtimeMode
    ? { background: '#e0e5ec' }
        ...
        {isCollapsed ? (
          ...
          >
              ...
              {showSidebar && (
                <motion.div
                  ...
                  className={`absolute inset-0 z-10 flex flex-col rounded-2xl overflow-hidden ${
                    bedtimeMode ? 'bg-[#e0e5ec]' : 'bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl'
                  }`}
                >
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
              ...
              {/* Header */}
              <div className={`flex items-center justify-between p-3 border-b flex-shrink-0 ${borderStyle} ${
                bedtimeMode ? 'bg-[#e0e5ec]' : 'bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm'
              }`}>
              ...
              {/* Messages Area — background gradient applied here via style prop */}
              <div
              ...
              >
                {messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center p-6">
                    ...
                    <p className={`text-sm font-medium mb-1 ${headingStyle}`}>
                      Welcome to AI Shine!
                    </p>
                    <p className={`text-xs ${textMuted}`}>
                      Ask me anything about AI & technology
                    </p>
                  </div>
                ) : (
                  <AnimatePresence>
              ...
              {/* Input Area */}
              <div className={`p-2 border-t flex-shrink-0 ${borderStyle} ${
                bedtimeMode ? 'bg-[#e0e5ec]' : 'bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm'
              }`}>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    disabled={loading}
                    className={`flex-1 px-3 py-2 rounded-xl text-xs outline-none transition-all min-w-0 ${inputStyle} ${
                      loading ? 'opacity-50' : ''
                    }`}
                  />
                      ...
                      className={`p-2 rounded-lg transition-all cursor-pointer flex-shrink-0 ${
                        listening
                          ...
                          : bedtimeMode
                            ...
                            : 'bg-white/80 dark:bg-gray-700 text-gray-600 dark:text-gray-300'
                      }`}
                    >
          ...
          /* ── Collapsed icon bubble — non-widget minimize mode only ── */
          !isWidgetMode && (
